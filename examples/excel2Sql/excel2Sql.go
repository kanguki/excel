...
func Do() {
	...
	result := make(chan string) //increase channel capacity doesn't really improve much :D
	...
	for _, v := range data {
		convert(result, v) //no need to use goroutine here. it's not parallel.
	}
	wg.Wait()
...
func convert(in chan string, data []string) {
	var info string
	defer func() {
		in <- info
	}()
